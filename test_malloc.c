...
void *dowork(void *threadid) {
    ...
    for (i = 0; i < ops; i++) {
        switch(tr.ops[i].type) {
            case MALLOC:
              debug_print("thread%li: malloc block %d (size %d, filled with %c)\n", id, tr.ops[i].index, tr.ops[i].size, 64 +(tr.ops[i].index)%59);
                ...
                debug_print("\t block %d malloced at address %p\n",
                            tr.ops[i].index, tr.blocks[tr.ops[i].index]);
                for (iter = 0; iter*sizeof(char) < tr.ops[i].size; iter++){
                  char * temp = (tr.blocks[tr.ops[i].index] + iter);
                  *temp = 64 +(tr.ops[i].index)%59;
                  //debug_print("\t index %d in block %d set to %c \n", iter, tr.ops[i].index, *temp);
                }
				check_heap();
                break;
            ...
            case FREE:
                ...
                debug_print("thread%li: free block %d located at %p : \n",
                          id, tr.ops[i].index, ptr);
#if DEBUG
                for (iter = 0; iter < MAX_BLOCKSIZE; iter++){
//printf("\t index %d in block %d of size %d is %c \n", iter, tr.ops[i].index, blocksize, *(ptr + iter));
                  //if (iter > 2038){
                  if (*(ptr+iter) != (64 +(tr.ops[i].index)%59)){
                    printf("\tEnd of block %d; size on free = %d \n", tr.ops[i].index, iter);
                    printf("\tExpected %c, got %c \n", 64 +(tr.ops[i].index)%59, *(ptr +iter));
                    break;
                  }
                }
#endif
